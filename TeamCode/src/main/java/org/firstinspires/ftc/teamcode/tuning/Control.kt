...
class HolonomicController(
    ...
    constructor(
        ...
        headingGain: Double,
    ) : this(axialPosGain, lateralPosGain, headingGain, 0.0, 0.0, 0.0, doubleArrayOf(0.0),
     ...
     */
    fun getValueForTime(t: Double, times: DoubleArray, values: DoubleArray): Double {
        // Ensure t is within the valid range [0, 1]
        if (t < 0 || t > 1) {
            throw IllegalArgumentException("t must be between 0 and 1")
        }

        // Find the interval that t falls into
        for (i in 0 until times.size - 1) {
            if (t >= times[i] && t <= times[i + 1]) {
                ...
                // Interpolate using the formula: y = y1 + (t - x1) * (y2 - y1) / (x2 - x1)
                return y1 + (t - x1) * (y2 - y1) / (x2 - x1)
            }
        }

        // If t is exactly 1.0, return the last value in the list
        return values.last()
    }
    fun compute(
        ...
        val pcalculatedHError = targetPose.heading.value().real+getValueForTime(speed,hS,hk);
        elapsed+=hI*0.1;
        i+= pcalculatedHError*elapsed;
        e = pcalculatedHError
        ...
        return targetVelTarget +
                PoseVelocity2d(
                    ...
                    headingGain * error.heading.log() + pcalculatedHError * hP + i*hI + ((pcalculatedHError-e)*elapsed) * hD * error.heading.log(),
                ) +
                PoseVelocity2d(
                    Vector2d(
